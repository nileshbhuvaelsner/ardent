(function (api) {
    ...
    api.sectionConstructor['ardent-plus'] = api.Section.extend({
        ...
        _init: function () {
            ...
            control.actions = function ($context) {
                if (control.params.live_title_id) {
                    ...
                    if (control.id === 'ardent_section_order_styling') {
            ...
            control.container.on('click', '.add-new-repeat-item', function () {
				...
				if ( "ardent_map_items_address" === controlbox_id ) {
					var map_long = wp.customize( 'ardent_map_long' ).get();
					var map_lat = wp.customize( 'ardent_map_lat' ).get();
					if ( '' === map_long || '' === map_lat ) {
						$('#customize-control-ardent_map_items_address').find('label').append( '<span class="ardent-customizer-notice">'+ARDENT_CUSTOMIZER_DATA.multiple_map_notice+'</span>' );
						return;
					} else {
						$('#customize-control-ardent_map_items_address').find('.ardent-customizer-notice').remove();
					}
				}
    ...
    function _the_editor(container) {
        var _editor = {
            ...
            ready: function (container) {
                ...
                control.container.addClass('ardent-editor-added');
...
jQuery(window).ready(function ($) {
    ...
    if (typeof ardent_customizer_settings !== "undefined") {
        if (ardent_customizer_settings.number_action > 0) {
            $('.control-section-themes h3.accordion-section-title').append('<a class="theme-action-count" href="' + ardent_customizer_settings.action_url + '">' + ardent_customizer_settings.number_action + '</a>');
        }
    }
     ...
     */
    $('select[data-customize-setting-link="ardent_hero_layout"]').on('change on_custom_load', function () {
        var v = $(this).val() || '';

        $("li[id^='customize-control-ardent_hcl']").hide();
        $("li[id^='customize-control-ardent_hcl" + v + "']").show();

    });
    $('select[data-customize-setting-link="ardent_hero_layout"]').trigger('on_custom_load');


    /**
     * For Gallery content settings
     */
    $('select[data-customize-setting-link="ardent_gallery_source"]').on('change on_custom_load', function () {
        var v = $(this).val() || '';

        $("li[id^='customize-control-ardent_gallery_source_']").hide();
        $("li[id^='customize-control-ardent_gallery_api_']").hide();
        $("li[id^='customize-control-ardent_gallery_settings_']").hide();
        $("li[id^='customize-control-ardent_gallery_source_" + v + "']").show();
        $("li[id^='customize-control-ardent_gallery_api_" + v + "']").show();
        $("li[id^='customize-control-ardent_gallery_settings_" + v + "']").show();

    });

    $('select[data-customize-setting-link="ardent_gallery_source"]').trigger('on_custom_load');

    /**
     * For Gallery display settings
     */
    $('select[data-customize-setting-link="ardent_gallery_display"]').on('change on_custom_load', function () {
        var v = $(this).val() || '';
        switch (v) {
            case 'slider':
                $("#customize-control-ardent_g_row_height, #customize-control-ardent_g_col, #customize-control-ardent_g_spacing").hide();
                break;
            case 'justified':
                $("#customize-control-ardent_g_col, #customize-control-ardent_g_spacing").hide();
                $("#customize-control-ardent_g_row_height").show();
                break;
            case 'carousel':
                $("#customize-control-ardent_g_row_height, #customize-control-ardent_g_col").hide();
                $("#customize-control-ardent_g_col, #customize-control-ardent_g_spacing").show();
                break;
            case 'masonry':
                $("#customize-control-ardent_g_row_height").hide();
                $("#customize-control-ardent_g_col, #customize-control-ardent_g_spacing").show();
                break;
            default:
                $("#customize-control-ardent_g_row_height").hide();
    ...
    $('select[data-customize-setting-link="ardent_gallery_display"]').trigger('on_custom_load');
